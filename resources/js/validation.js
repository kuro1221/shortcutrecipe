export default {
    ...
    limit_min_length: value => value.length >= 8 || "8文字以上で入力してください", // 文字数の制約
    limit_max_length: value => value.length <= 50 || "50文字以内で入力してください", // 文字数の制約
    limit_max_comment: value => value.length <= 200 || "200文字以内で入力してください", //文字数の制約
    select: value => !!value || "必ず選択してください",
    email: v => !!v.match(/^[a-zA-Z0-9.!#$%&'*+\/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/) || '正しい形式で入力してください',
    tel: value => !!value.match(/^(0[5-9]0[0-9]{8}|0[1-9][1-9][0-9]{7})$/) || '電話番号を正しく入力してください',

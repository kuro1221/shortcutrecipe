    ...
    <v-content class="fill-height pt-0" red >
      <v-row justify="center" red class="pt-0">
        <v-col cols="12" sm="8" md="9">
          <recipe-filter :filter_data.sync="filter_data" :search.sync="search" :sort_order.sync="sort_order" :application_list="application_list" :product_list="product_list"></recipe-filter>
    ...
    filterdList: function () {
      ...
      this.recipes.forEach(recipe => {
        ...
        if(this.filter_data.filter_product.length > 0) {
          if(!this.filterSelection(this.filter_data.filter_product,recipe.products,"id")){
            isShow = false;
          }
        }

        if(this.filter_data.filter_application.length > 0) {
          if(!this.filterSelection(this.filter_data.filter_application,recipe.applications,"id")){
            isShow = false;
          }
        }
        //検索値からフィルタリング
        if (this.search.length > 0) {
          ...
          (recipe.comment && recipe.comment.match(this.search)) ||
            this.filterSelection(this.search,recipe.products,"product_name") ||
            this.filterSelection(this.search,recipe.applications,"application_name")
          )){
            isShow = false;
          }
        }

    ...
    <v-row justify="center">
        ...
        >
            <v-card class="pa-2">
                <div v-if="showURL">
                    <v-card-text
                        >これより外部のサイトにアクセスします。よろしければ下記リンク先のURLをクリックして下さい。<br />ショートカットをダウンロードする前に安全性を確認してください。
                    </v-card-text>
                    <v-card-text class="pt-0"
                        ><a :href="recipe.iCloud_link">{{
                            recipe.iCloud_link
                        }}</a></v-card-text
                    >
                </div>
                ...
                <div v-else>
                    ...
                    <v-card-text
                        class="px-2 py-1 button text--secondary font-weight-bold"
                        >{{ recipe.comment }}</v-card-text
                    >
                    <v-card-text
                        class="px-2 py-1 button text--secondary font-weight-bold"
                        ><a
                    ...
                    <v-chip
                        ...
                        outlined
                        pill
                        v-for="(select_applications,
                        key,
                        index) in recipe.applications"
                        :key="index"
                    ...
                    <v-card-actions>
                        ...
                        <v-card-text class="ml-1 font-weight-bold text-truncate"
                            ><a
                                class="link"
                                :href="'/userDetail/' + recipe.user_id"
                                >{{ recipe.name }}</a
                            ></v-card-text
                        >
                    </v-card-actions>
                    <v-card-actions>
                        <v-spacer></v-spacer>
                        <v-btn
                            class="button_small ml-3"
                            color="#FFD500"
                            rounded
                            small
                            v-if="recipe.user_id === auth_user.id"
                        ...
                        <v-btn
                            ...
                            small
                            v-if="recipe.user_id === auth_user.id"
                            @click="deleteRecipeId = recipe.id"
                            >削除</v-btn
                        >
                        <v-btn
                            class="button_small ml-3"
                            ...
                            >GET</v-btn
                        >
                        <v-icon class="ml-2" @click="modalFlg = false"
                            >mdi-close</v-icon
                        >
                    </v-card-actions>
                </div>
